...
AFRAME.registerComponent('beat-system', {
  ...
  updateVerticalPositions: (function () {
    const BASELINE_CAMERA_HEIGHT = 1.55;
    const BASELINES = {
      bottom: 0.8,
      middle: 1.25,
      top: 1.65
    ...
    return function () {
      const offset = this.el.sceneEl.camera.parent.position.y - BASELINE_CAMERA_HEIGHT;
      this.verticalPositions.bottom = Math.max(MIN.BOTTOM, BASELINES.bottom + offset);
      this.verticalPositions.middle = Math.max(MIN.MIDDLE, BASELINES.middle + offset);
      this.verticalPositions.top = Math.max(MIN.TOP, BASELINES.top + offset);
    };
  })()
});
